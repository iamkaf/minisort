...
public final class RefillService {
    ...
    private static int findReplacement(
        ...
        if (RefillConfig.SEARCH_HOTBAR_FIRST.get()) {
    ...
    enum RefillCategory {
        ...
        boolean enabled() {
            return switch (this) {
                case BLOCK -> RefillConfig.REFILL_BLOCKS.get();
                case TOOL -> RefillConfig.REFILL_TOOLS.get();
                case CONSUMABLE -> RefillConfig.REFILL_CONSUMABLES.get();
                case GENERIC -> RefillConfig.REFILL_GENERIC_USE_ITEMS.get();
            };
        }
    }
}
